...
import (
	...
	postgresEventStore "github.com/looplab/eventhorizon/eventstore/postgres"
...
guest list: 4 invited - 3 accepted, 1 declined - 2 confirmed, 1 denied`)
	...
	if err := rootDB.Close(); err != nil {
		log.Fatal("could not close DB:", err)
	}
	...
	eventStore, err := postgresEventStore.NewEventStore(uri,
		postgresEventStore.WithEventHandler(eventBus), // Add the event bus as a handler after save.
	)
	if err != nil {
	...
	// TODO: Use Postgres repos.
	mongoAddr := os.Getenv("MONGODB_ADDR")
	if mongoAddr == "" {
		mongoAddr = "localhost:27017"
	}
	url := "mongodb://" + mongoAddr

	// Create the read repositories.
	invitationRepo, err := mongodb.NewRepo(url, db, "invitations")
	if err != nil {
	...
	guestListRepo, err := mongodb.NewRepo(url, db, "guest_lists")
	if err != nil {
